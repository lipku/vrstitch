...
NVENCSTATUS CNvHWEncoder::CreateEncoder(EncodeConfig *pEncCfg)
{
	...
	m_cbFunc = pEncCfg->cbfunc; //lihengz
	m_cbContext = pEncCfg->cbcontext; //lihengz
    ...
    m_stCreateEncodeParams.enableEncodeAsync = 0; // asyncMode; lihengz

    pEncCfg->enableAsyncMode = 0; // asyncMode; lihengz
...
NVENCSTATUS CNvHWEncoder::ProcessOutput(const EncodeBuffer *pEncodeBuffer)
{
    ...
    if (nvStatus == NV_ENC_SUCCESS)
    {
		...
		if (m_cbFunc && m_cbContext) //lihengz
		{
			m_cbFunc((unsigned char*)lockBitstreamData.bitstreamBufferPtr, lockBitstreamData.bitstreamSizeInBytes,
				lockBitstreamData.outputTimeStamp, m_cbContext);
		}
