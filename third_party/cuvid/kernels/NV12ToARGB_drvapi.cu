                                  ...
                                  uint32 width,         uint32 height)
{
    ...
    if (x >= width-1) //lihengz
        return; //x = width - 1;

    if (y >= height-1)  //lihengz
        return; // y = height - 1;
    ...
    // Clamp the results to RGBA
	dstImage[y * dstImagePitch + x] =  RGBAPACK_10bit(red[0], green[0], blue[0], constAlpha); //lihengz
	dstImage[y * dstImagePitch + x + 1] =  RGBAPACK_10bit(red[1], green[1], blue[1], constAlpha); //lihengz
